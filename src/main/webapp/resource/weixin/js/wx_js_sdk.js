...
wx.ready(function(){
	...
	//---------------	音频接口	-----------------------
	//开始录音接口
	$("#startRecord").on(function(){
		wx.startRecord();
	});
	
	//停止录音接口
	$("#stopRecord").on(function(){
		wx.stopRecord({
		    success: function (res) {
		        var localId = res.localId;
		    }
	...
	//---------------	摇一摇周边	-----------------------
	//开启查找周边ibeacon设备接口
	$("#startSearchBeacons").on(function(){
		wx.startSearchBeacons({
			ticket:"",  //摇周边的业务ticket, 系统自动添加在摇出来的页面链接后面
			complete:function(argv){
				//开启查找完成后的回调函数
	...
	//---------------	界面操作	-----------------------
	//隐藏右上角菜单接口
	$("#hideOptionMenu").on(function(){
		wx.hideOptionMenu();
	});
	
	//显示右上角菜单接口
	$("#showOptionMenu").on(function(){
		wx.showOptionMenu();
	});
	
	//关闭当前网页窗口接口
	$("#closeWindow").on(function(){
		wx.closeWindow();
	});
	
	//批量隐藏功能按钮接口
	$("#hideMenuItems").on(function(){
		wx.hideMenuItems({
	    	menuList: [] // 要隐藏的菜单项，只能隐藏“传播类”和“保护类”按钮，所有menu项见附录3
		});
	});
	...
	//隐藏所有非基础按钮接口
	$("#hideAllNonBaseMenuItem").on(function(){
		wx.hideAllNonBaseMenuItem();
		// “基本类”按钮详见附录3
	});

	//显示所有功能按钮接口
	$("#showAllNonBaseMenuItem").on(function(){
		wx.showAllNonBaseMenuItem();
	});
	...
	//---------------	微信小店	-----------------------
	//跳转微信商品页接口
	$("#openProductSpecificView").on(function(){
		wx.openProductSpecificView({
		    productId: '', // 商品id
		    viewType: '' // 0.默认值，普通商品详情页1.扫一扫商品详情页2.小店商品详情页
		});
	...
	//核销后再次赠送卡券接口
	$("#consumeAndShareCard").on(function(){
		wx.consumeAndShareCard({
		    cardId: '',
		    code: ''
		});
